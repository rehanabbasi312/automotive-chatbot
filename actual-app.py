...
from db import insert_user, is_email_exists, verify_user, get_db_connection, get_all_makes, get_cars_by_make, get_all_years, get_cars_by_year, get_all_fuel, get_cars_by_fuel, get_all_colors, get_cars_by_colors, get_all_miles, get_cars_by_mileage
...
def get_car_query(user_message, chat_id):
    ...
    query_map = {
        'used cars': "SELECT * FROM inventory WHERE new_used = 'Used' AND make IS NOT NULL AND model IS NOT NULL AND engine IS NOT NULL AND mileage IS NOT NULL AND fuel IS NOT NULL AND year IS NOT NULL AND imagelist IS NOT NULL AND imagelist <> '' ORDER BY RAND() LIMIT 3",
        'cheapest cars': "SELECT * FROM inventory WHERE price IS NOT NULL AND make IS NOT NULL AND model IS NOT NULL AND engine IS NOT NULL AND mileage IS NOT NULL AND fuel IS NOT NULL AND year IS NOT NULL AND new_used IS NOT NULL AND imagelist IS NOT NULL AND imagelist <> '' ORDER BY price ASC LIMIT 3",
        'latest year cars': "SELECT * FROM inventory WHERE year IS NOT NULL AND make IS NOT NULL AND model IS NOT NULL AND engine IS NOT NULL AND mileage IS NOT NULL AND fuel IS NOT NULL AND new_used IS NOT NULL AND imagelist IS NOT NULL AND imagelist <> '' ORDER BY year DESC LIMIT 3"
    }
    ...
    if "model" in car_preferences[chat_id]:
        ...
        query = """
            SELECT new_used, year, make, model, body_style, doors, trim, ext_color, int_color, 
...
def index():
    ...
    print("Makeeeeeeeeeeeeeeee")  # Debugging print to verify data
...
def vpd(make):
    cars=get_cars_by_make(make)
    return render_template('vdp.html', make=make, cars=cars)
